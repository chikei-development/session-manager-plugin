...
func getUuid(log log.T, byteArray []byte, offset int) (result uuid.UUID, err error) {
	...
	if err != nil {
		log.Error("getUuid failed: Failed to convert bytes to uuid.")
		return uuid.Nil, errors.New("Failed to convert bytes to uuid.")
	}
...
func SerializeClientMessagePayload(log log.T, obj any) (reply []byte, err error) {
